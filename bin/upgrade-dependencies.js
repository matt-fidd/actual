...
async function getAllUpdates() {
  ...
  try {
    ...
    if (existsSync(configPath)) {
      ...
      userCfg = require(path.resolve(process.cwd(), configPath));
    }
    ...
    console.warn(`Warning: Failed to get updates: ${String(error)}`);
    return [];
  }
}
...
function commitUpdate(groupedUpdate) {
  ...
  try {
    ...
    throw new Error(`Failed to commit: ${String(error)}`);
  }
}
...
function verifyUpdate() {
  ...
  try {
    ...
    console.log('  Running yarn test...');
    execAsync('yarn test', undefined);
